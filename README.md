...
This bundle implements the server side row model logic according to [the Ag Grid documentation](https://www.ag-grid.com/javascript-data-grid/server-side-model/), exposing a single service that takes care of retrieving rows according to the request body automatically generated by Ag Grid, and returns a Response object with the expected format.
## Usage
